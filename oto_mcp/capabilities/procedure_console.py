...
CAPABILITIES += [
    Capability(
        ...
        description=(
            ...
            "comes WITHOUT its flowchart, one `<!-- flowchart: … -->` line standing in — "
            "see `set` for what brings it back; `full=true` reads it, and the catalog "
            ...
            "box characters): the response carries `diagram_warning` when the body has "
            "none, and says so too when TWO drawings ended up in it. op=get serves that "
            "flowchart as one `<!-- flowchart: … -->` line — keep it VERBATIM and op=set "
            "puts the drawing back, but ONLY on the same slug and scope you read from: "
            "op=create, a new slug, or an omitted `scope` (= `user`) drop it. "
            ...
            "which is why it is written here. Flowchart placement: right after the "
            "« At a glance » table (or the intro), before the first phase heading; it is "
            "the DEFAULT view of the process page and its grammar is a contract — read "
            "the `procedure-flowchart` guide before writing one. ⚠️ The org README (the "
