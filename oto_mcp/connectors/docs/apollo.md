...
- **le reveal de TÉLÉPHONE est BYO-only lui aussi, et pour une autre raison : le
  COÛT.** Apollo facture ~9 crédits un appel qui rend un mobile, là où un match nu en
  coûte 1 — le pot commun ne le finance pas. `apollo_match_person` continue de marcher
  sans ta clé, il ne rend simplement aucun mobile ni direct dial.
...
## usage — téléphone direct (mobile / direct dial)
...
- `apollo_match_person(reveal_personal_emails=True)` — les emails PERSONNELS, eux,
  reviennent bien dans la réponse (synchrone). apollo les retient pour les personnes en
  zone RGPD : un résultat vide est une réponse, pas une panne.
