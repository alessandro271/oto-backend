...
def _kwargs_called_on_client(tree: ast.Module) -> dict[str, set[str]]:
    ...
    du receveur que `_methods_called_on_client`.

    Un appel qui déballe (`m(**opts)`) est ignoré : ce qu'il porte n'est pas
    lisible statiquement, et prétendre le vérifier vaudrait moins que dire qu'on
    ne le vérifie pas.
    """
    bound = _names_bound_to_client(tree) | {"c"}
    ...
    for node in ast.walk(tree):
        ...
        est_client = (
            (isinstance(recv, ast.Call) and isinstance(recv.func, ast.Name)
             and recv.func.id in _CLIENT_FACTORIES)
            or (isinstance(recv, ast.Name) and recv.id in bound))
        if not est_client:
            continue
