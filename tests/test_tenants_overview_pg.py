...
- **les deux sources restent séparées** — un compte qualifié `acme:…` compte pour
  Acme même quand son organisation est restée sur le tenant `oto`, et cet écart
...
def base(pg_module_dsn, monkeypatch):
    ...
    with psycopg.connect(pg_module_dsn, row_factory=dict_row, autocommit=True) as c:
        ...
        c.execute("INSERT INTO tenants (slug, name, issuer) VALUES "
                  ...
                  "('globex', 'Globex', NULL) RETURNING id")
        acme = c.execute("SELECT id FROM tenants WHERE slug='acme'").fetchone()["id"]
        oto = c.execute("SELECT id FROM tenants WHERE slug='oto'").fetchone()["id"]

        # 2 comptes nus (tenant oto) + 2 comptes qualifiés (tenant acme), dont un
        # qui n'a jamais rien appelé — l'inactif est celui qu'un suivi doit montrer.
        for sub in ("alice", "bob", "acme:carla", "acme:dan"):
            c.execute("INSERT INTO users (sub, email) VALUES (%s, %s)",
                      (sub, f"{sub}@ex.test"))

        # Une org par tenant, + l'org DÉSALIGNÉE : rattachée au tenant `oto`, créée
        # par un compte qualifié `acme:` (l'état que laisse une bascule partielle).
        c.execute("INSERT INTO orgs (name, created_by, tenant_id) VALUES "
                  "('Oto Team', 'alice', %s), ('Acme', 'acme:carla', %s), "
                  "('Reprise', 'acme:dan', %s)", (oto, acme, oto))
...
def test_les_compteurs_dun_tenant(base):
    ...
    assert set(par) == {"oto", "acme", "globex"}
    ...
    assert oto["orgs_desalignees"] == 1           # « Reprise », créée par acme:dan

    acme = par["acme"]
    assert acme["orgs"] == 1
    assert acme["comptes"] == 2                 # carla + dan, qualifiés
    assert (acme["appels"], acme["comptes_actifs"]) == (1, 1)  # le 'rest' exclu
    assert acme["orgs_desalignees"] == 0
...
def test_un_tenant_sans_aucun_compte_reste_une_ligne(base):
    ...
    from oto_mcp import db
    globex = _par_slug(db.list_tenants_overview(days=30))["globex"]
    assert (globex["comptes"], globex["orgs"], globex["appels"]) == (0, 0, 0)
    assert globex["authenticates"] is False   # aucune ligne d'émetteur
...
def test_la_classification_sql_dit_la_meme_chose_que_le_registre(base):
    ...
    assert sorted(subs) == ["acme:carla", "acme:dan"]
    assert all(registre.tenant_of(s) == "acme" for s in subs)
...
def test_la_fiche_donne_ladresse_de_lecart(base):
    ...
    assert [(o["name"], o["tenant_du_createur"]) for o in ecart] == [("Reprise", "acme")]
